...
int i;
string ch;//当前分析字符
string s, str, stackStr;//s输入串;str中间变量;stackStr模拟栈
...
void push(string pre, string value) {
    ...
    if (value != "ε")//不是空字时
    {
        string value1;  //中间变量,用于存储value
        value1 = value;
        if (value[0] == '+')    value1 = "TG";
        if (value[0] == '*')    value1 = "FS";
        if (value[0] == '(')    value1 = "E";
        if (value[0] == 'i')    value1 = "";
        stackStr.replace(idx, 1, value1);//将第一个pre的位置替换为value1
    } else {
        stackStr.erase(idx, 1);//删除从该位置开始的1个字符,即删除pre    
    }
